...
def get_CosmoPowerJAX_pars_to_theory_specs_func(emulator_filenames):
    ...
    for spec_type in list(emulator_filenames.keys()):
        if spec_type == "TE":
            try:
                ...
                print("TE emulator loaded from default directory:")
                print("File:", emulator_filenames[spec_type])
            except:
                cp_emulators[spec_type] = CPJ(
                    probe="custom_pca", filepath=emulator_filenames[spec_type]
                )
                print("TE emulator loaded from file path:")
                print("File:", emulator_filenames[spec_type])
        else:
            try:
                cp_emulators[spec_type] = CPJ(
    ...
    def pars_to_theory_specs(pars, ell_high_cut, ell_low_cut=2):
        ...
        for spec_type in list(cp_emulators.keys()):
            ...
            theory_start_ix = int(theory_start_ix)
            theory_stop_ix = int(theory_stop_ix)

            like_start_ix = int(like_start_ix)
            like_stop_ix = int(like_stop_ix)
            ...
            print(pars_for_cp)
            print(cp_emulators[spec_type].predict(pars_for_cp))
